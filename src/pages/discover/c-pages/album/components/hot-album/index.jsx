import React, { memo, useEffect } from 'react'
import { useDispatch, useSelector, shallowEqual } from 'react-redux'

import { fetchHotAlbumsAction } from '../../store/slice'

import CYAlbumCover from '@/components/album-cover'
import CYThemHeaderNormal from '@/components/theme-header-normal'
import { HotAlbumWrapper } from './style'

export default memo(function CYHotAlbum() {
  const { hotAlbums } = useSelector(
    state => ({
      hotAlbums: state.album.hotAlbums,
    }),
    shallowEqual
  )
  const dispatch = useDispatch()

  useEffect(() => {
    dispatch(fetchHotAlbumsAction())
  }, [dispatch])

  return (
    <HotAlbumWrapper>
      <CYThemHeaderNormal title='热门新碟' />
      <div className='album-list'>
        {hotAlbums.slice(0, 10).map((item, index) => {
          return (
            <CYAlbumCover
              size={'130px'}
              width={'153px'}
              bgp={'-845px'}
              key={item.id}
              info={item}
            />
          )
        })}
      </div>
    </HotAlbumWrapper>
  )
})
